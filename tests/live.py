...
FILES=['index.html','styles.css','src/model.js','src/defenses.js','src/data.js','src/engine.js','src/app.js','src/storage.js','src/renderer.js','src/audio.js','manifest.webmanifest','sw.js','icon.png','update.html','release.json']
def remote(path):
 req=Request(BASE+path+'?verify=1.2.0',headers={'User-Agent':'Deadblock-live-check','Cache-Control':'no-cache'})
...
for attempt in range(40):
 ...
 time.sleep(10)
else:raise RuntimeError('Pages did not publish version 1.2.0 in time')
assets=[]
for path in FILES:
...
with sync_playwright() as pw:
 ...
 page.goto(BASE+'?v=1.2.0');page.bring_to_front();expect(page).to_have_title('Deadblock | Last stand')
 assert page.evaluate('typeof window.__deadblock')=='undefined'
 page.wait_for_function("document.getElementById('offlineStatus').textContent.includes('1.2.0')")
 ...
 report={'url':BASE,'version':'1.2.0','passed':True,'assets':assets,'browser':'WebKit','checks':['15 live assets match repository','landscape rotation guard','first-wave preparation','buy structure wall','intermission exits to prep','prep save resumes before next wave','new game starts','manual idle does not shoot','compact five-button HUD','More opens and closes','select mine','single saved run resumes','production has no test hooks','zero page errors']}
