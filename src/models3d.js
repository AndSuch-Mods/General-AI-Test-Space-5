...
export function createModelBlueprint(kind='walker', weapon='pistol') {
 ...
 box('head','jaw',0,human?-3.5:-7.3,2,hw-3,4,21,demon?p.cloth:p.skin,1.1);
 for(const side of [-1,1]){
  ...
  box('head','eye-socket',side*5.5,10.5,12.1,7.5,5.8,1,'#302a21',.4);
  if(human)box('head','eye-white',side*5.5,10.5,12.8,5.8,3.6,.3,'#dcdac4',.08);
  box('head','eye',side*5.5,10.5,13.25,human?2.8:4.9,3.3,.32,human?'#291e18':demon?'#ff422b':'#ffdd73',.09,[0,0,0],human?'matte':'glow');
  box('head','brow',side*5.5,15,12.1,8,2.5,1.8,demon?p.dark:human?p.hair:p.skin,.5,[0,0,side*.10]);
 }
 box('head','nose',0,7.8,12.5,human?4.4:4,5,3.8,demon?p.light:p.skin,.6);
 if(human){
  box('head','muzzle-plane',0,1.4,11.5,13,6,3,p.skin,.55);
  ...
  box('head','mouth-cavity',0,-.6,11.55,demon?14:12,11.5,1.2,'#241715',.2);
  for(const row of [0,1])for(let i=0;i<(demon?6:4);i++){
   if(!demon&&((i+row)%5===1))continue;
   box('head','tooth',-(demon?6.3:4.8)+i*(demon?2.5:3.3),row?3.8:-5,12.6,1.8,demon?2.8:2.2,1.3,TOOTH,.2);
  }
  tile('head','tongue',0,-3.3,12.3,3.1,1.8,'#9e3326');
  if(!demon){
...
export function compileModel(blueprint){
 ...
 for(const piece of blueprint.pieces){
  ...
  if(!shapeCache.has(shapeKey))shapeCache.set(shapeKey,piece.shape==='microbox'?new THREE.BoxGeometry(...piece.size).toNonIndexed():piece.shape==='cone'?new THREE.ConeGeometry(piece.size[0],piece.size[1],4).toNonIndexed():beveledBox(piece.size,piece.bevel??.3));
